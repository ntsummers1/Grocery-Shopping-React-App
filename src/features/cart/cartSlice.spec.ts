...
import cartReducer, { addToCart, ICartState, removeFromCart } from "./cartSlice"
...
describe('Cart Reducer', () => { 
  ...
  const twoItemInCartState : ICartState = {
    status: 'idle',
    items: [{...productOne, qty: 1}, {...productTwo, qty: 1}]
  }

  const twoItemWithMultipleQtyInCartState : ICartState = {
    status: 'idle',
    items: [{...productOne, qty: 1}, {...productTwo, qty: 2}]
  }


  it('should handle default state', () => {
    expect(cartReducer(undefined, {type: undefined})).toEqual({status: "idle", items: []})
  })
  ...
  it('should lower the quantity of one item in the cart', () => {
    const cart = cartReducer(twoItemWithMultipleQtyInCartState, removeFromCart(productTwo));
    expect(cart.items).toEqual([{...productOne, qty: 1}, {...productTwo, qty: 1}])
  })
