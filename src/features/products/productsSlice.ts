...
const productsSlice = createSlice({
  ...
  reducers: {
    filterProducts: (state, action: PayloadAction<IProductsFilter>) => {
      ...
      if (filters.instock) {
        state.filteredProducts = state.filteredProducts.filter(item => item.instock === filters.instock )
      }
      if (filters.name) {
        state.filteredProducts = state.filteredProducts.filter(item => item.name.includes(filters.name))
      }
    }
  },
